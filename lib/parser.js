...
var parse = module.exports.parse = function(str) {
    ...
    _.each(str, function(c) {
        if (state.ESCAPE_NEXT) {
            ...
            // put this character in verbatim and reset the escape status
            state.CURR_ARG += c;
    ...
    if (state.ESCAPE_NEXT) {
        // If it ended with an escape character, we exclude it since it is escaping the new line in
        // such a way that we don't retain the new line in the input and we also don't escape the
        // first character of the next line
        return {'str': str.slice(0, -1), 'closed': false};
    } else if (state.DOUBLE_QUOTE || state.SINGLE_QUOTE) {
        // When we finished while quoting, we retain the new line at the end of the current segment
        // of the input string. So append it and continue on
        return {'str': str + '\n', 'closed': false};
    } else {
